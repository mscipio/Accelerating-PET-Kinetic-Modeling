function [IF, IFpar] = FengInput(params, t)
...
switch modelNumber
	case 2
      ...
      IF = zeros(size(t));
      idx = find(t > delay);
      IF(idx) = (a(1)*(t(idx)-delay)-a(2)-a(3)).*exp(lambda(1)*(t(idx)-delay)) + ...
   ...
   case 4
      ...
      IF = zeros(size(t));
      idx = find(t > delay);
      IF(idx) = (a(1)*(t(idx)-delay)-a(2)).*exp(lambda(1)*(t(idx)-delay)) + ...
               a(2)*exp(lambda(2)*(t(idx)-delay)); 
   ...
   case 5
      ...
      IF=zeros(size(t));
      idx=find(t > delay);
      IF(idx) = a(1)*exp(lambda(1)*(t(idx)-delay)) + ...
